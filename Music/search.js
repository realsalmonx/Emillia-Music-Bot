...
async execute(message,args,client) {
    ...
    message.react("822857547014209536").catch(console.error);
    ...
    let resultsEmbed = new MessageEmbed()
      .setTitle("<:Playing:822858092496683028> Results for: ")
    ...
    try {
      ...
      //react with 5 Numbers
        await resultsMessage.react("822857749284126721");
        await resultsMessage.react("822857726236426280");
        await resultsMessage.react("822857692179726337");
        await resultsMessage.react("822857666136244244");
        await resultsMessage.react("822857619403964416");
      ...
      {max: 1, time: 60000, errors: ['time'],} ).then(collected => {
        //if its one of the emoji set them to 1 / 2 / 3 / 4 / 5
          if(collected.first().emoji.id == "822857749284126721"){ return response = 1; }
          if(collected.first().emoji.id == "822857726236426280"){ return response = 2; }
          if(collected.first().emoji.id == "822857692179726337"){ return response = 3; }
          if(collected.first().emoji.id == "822857666136244244"){ return response = 4; }
          if(collected.first().emoji.id == "822857619403964416"){ return response = 5; }
